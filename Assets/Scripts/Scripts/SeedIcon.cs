...
namespace ubco.hcilab.roadmap
{
    public class SeedIcon : MonoBehaviour
    {
        ...
        private void Start()
        {
            currentInventoryCount = InitManager.Instance.Items[_index].quantity;

            _hasSeeds = currentInventoryCount > 0;

            SetSeedIcon();

            UpdateLabel(InitManager.Instance.Items);
            InitManager.Instance.InventoryUpdated.AddListener(UpdateLabel);
        ...
        float NormalizedQuantity() {
            ...
            return (float)InitManager.Instance.Items[_index].quantity / (float)InitManager.Instance.StartingSeedQuantity;
        }
