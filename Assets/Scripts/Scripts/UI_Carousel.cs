...
namespace ubco.hcilab.roadmap
{
    ...
    public class UI_Carousel : MonoBehaviour//, IEndDragHandler, IBeginDragHandler
    {
       /*  /// Ref
        ...
        {
            if (InitManager.Instance.CurrentAppState == AppState.Watering) return;
        ...
        private void InstantiateIcons()
        {
            ...
            _distances = new float[InitManager.Instance.PlantPrefabs.Count];

            // Create Icons
            for (int i = 0; i < (int)InitManager.Instance.PlantPrefabs.Count; i++)
            {
                ...
                Plant plant = InitManager.Instance.PlantPrefabs[i].GetComponent<Plant>();
                if (plant != null) {
